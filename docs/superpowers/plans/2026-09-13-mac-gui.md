...
- [ ] **Step 9: Verify the command line against the real yt-dlp**

Every test above stubs `subprocess.run`, so none of them can catch a malformed yt-dlp
invocation. Check the real thing once, against a short Creative Commons video:
...
sys.argv = ["t", "-i", "/tmp"]
import tony
tony.parse_args(["-i", "/tmp"])
items = tony.probe_url("https://www.youtube.com/watch?v=aqz-KE-bpKQ")  # Big Buck Bunny
print("probe:", items[0]["title"], items[0]["duration"])
got = tony.download_url("https://www.youtube.com/watch?v=aqz-KE-bpKQ", "/tmp/ytcheck")
print("download:", got)
CHECK
```

Expected: `probe:` prints a title and a duration in seconds; `download:` prints one
entry whose `filepath` exists on disk and whose `title` is not `Untitled`.

If the `--print`/`--print-to-file`/`--no-simulate` combination does not produce parseable
JSON lines, adjust `download_url`'s command until it does, and record what you changed in
the report. This step is the only check on that command line.

Then clean up: `rm -rf /tmp/ytcheck`
...
`-i` goes last because `nargs="+"` consumes until the next flag. `argparse` treats an
argument after `-i` as a value when it does not match a known flag, which is what
`test_a_source_that_looks_like_a_flag_is_still_a_source` pins down.
...
(`upload_button`, `cancel_button`, `add_folder_button`, `add_files_button`,
`add_link_button`, `dry_run`, `force_update`, `max_duration`, `log`, `banner`,
`banner_label`), then
...
## Task 14: Tonie artwork and current chapters in the picker

**Files:**
- Create: `gui/tonie_images.py`
- Modify: `gui/app.py` — `_sign_in_job`, `_signed_in`, `set_tonies`
- Test: `tests/test_tonie_images.py` (create), `tests/test_gui_app.py` (extend)

**Interfaces:**
- Consumes: `tony.format_duration(seconds)`; the `CreativeTonie.imageUrl` and
  `CreativeTonie.chapters` fields already returned by `tony.get_all_creative_tonies`
- Produces: `gui.tonie_images.cache_dir() -> Path`; `cached_path(tonie_id, base_dir=None) ->
  Path`; `fetch_and_cache(tonie, base_dir=None) -> Path | None`; `fetch_all(tonies,
  base_dir=None) -> dict[str, Path | None]`; `MainWindow.set_tonies(tonies, households,
  image_paths=None)` (the `image_paths` parameter is new; the call gains a fourth
  argument, existing callers keep working since it defaults to `None`)

Both features are added together because they touch the same rows in `set_tonies`, and
splitting them would mean two diffs churning the same lines.

- [ ] **Step 1: Write the failing tests for the cache module**

Create `tests/test_tonie_images.py`:
...
Run: `pytest tests/test_tonie_images.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'gui.tonie_images'`

- [ ] **Step 3: Implement `gui/tonie_images.py`**
...
Run: `pytest tests/test_tonie_images.py -v`
Expected: PASS — 10 tests. `test_cache_dir_is_created_if_missing` touches the real
`~/Library/Caches` directory (creating it if absent, never writing a file inside it) -
this matches the house style of exercising a real OS facility rather than mocking it,
and creating a directory that would exist on any real run of the app is not residue
worth avoiding.

- [ ] **Step 5: Write the failing tests for the window integration**

Append to `tests/test_gui_app.py`. These need a `CreativeTonie`-shaped stub with
`chapters`; extend the existing `FakeTonie` helper used elsewhere in this file, or add a
local one if the file does not already export one with chapters attached:
...
def test_sign_in_fetches_images_for_every_tonie(window, configure, monkeypatch):
    ...
    monkeypatch.setattr("gui.app._sign_in_job", lambda u, p: (
        object(), [FakeTonie("Elephant")], {}))
...
If `FakeTonie` in `tests/test_gui_app.py` does not currently accept a `chapters=`
keyword or expose an `id` attribute, extend it rather than adding a second helper class -
every test in this file should build Tonies the same way.

- [ ] **Step 6: Run to verify they fail**

Run: `QT_QPA_PLATFORM=offscreen pytest tests/test_gui_app.py -k "chapter or image or tonie_images" -v`
Expected: FAIL — `set_tonies() takes 3 positional arguments but 4 were given`, and the
chapter tests fail because no child rows are created.

- [ ] **Step 7: Wire the fetch into sign-in**

In `gui/app.py`, extend `_sign_in_job`:
...
Add the import near the top of `gui/app.py`, alongside the other `gui` imports:
...
- [ ] **Step 8: Show chapters and icons in `set_tonies`**

Replace `set_tonies` with:
...
Add `QIcon` to the existing `PySide6.QtGui` (or wherever icons are already imported from
in this file) import line if it is not already imported.

- [ ] **Step 9: Run to verify they pass**

Run: `QT_QPA_PLATFORM=offscreen pytest tests/test_gui_app.py tests/test_tonie_images.py -v`
Expected: PASS

- [ ] **Step 10: Run the whole suite**

Run: `QT_QPA_PLATFORM=offscreen pytest tests/ -v`
Expected: PASS — the full suite plus these new tests, with nothing broken in
`set_tonies`'s existing callers (`select_needing_update`, `_tonie_rows`, checkbox
handling all read the row the same way as before; only new children and an optional icon
were added).

- [ ] **Step 11: Verify by hand against a real account, if one is available**

```bash
.venv-dev/bin/python -m gui
```

Sign in, and confirm: each Tonie shows its own picture next to its name (or no icon if
that Tonie has none), and clicking the expand arrow on a Tonie that holds chapters shows
their titles and durations. A Tonie with no chapters shows no arrow.

This step needs `tonie-api` installed and a real account; skip it if neither is
available and rely on the automated tests, noting in the report that it was skipped and
why.

- [ ] **Step 12: Commit**

```bash
git add gui/tonie_images.py gui/app.py tests/test_tonie_images.py tests/test_gui_app.py
git commit -m "Show each Tonie's own picture and its current chapters in the picker"
```

---

## Task 15: Find Homebrew-installed tools when launched from Finder

**Files:**
- Modify: `gui/__main__.py`
- Test: `tests/test_gui_main.py` (create)

**Interfaces:**
- Produces: `gui.__main__.augment_path(current=None, candidates=None) -> str` (a pure
  function, importable and testable without launching the real app)

**Why this exists.** A `.app` launched by double-clicking in Finder, from Launchpad, or
via `open` gets a minimal `PATH` from `launchd` — `/usr/bin:/bin:/usr/sbin:/sbin` — which
does not include Homebrew's install location. `tony.check_ffmpeg()` and
`tony.check_ytdlp()` both do a bare `subprocess.run(["ffmpeg", ...])`, relying on the OS
to resolve the command name through `PATH`; under the restricted `PATH`, resolution fails
and both report the tool as absent, even when it is genuinely installed. This was
confirmed directly: running the built app's own executable under
`env -i PATH="/usr/bin:/bin:/usr/sbin:/sbin"` reproduces it exactly —
`tony.check_ffmpeg()` and `tony.check_ytdlp()` both return `False` despite FFmpeg and
yt-dlp being installed and on the developer's normal shell `PATH`. Task 13's own launch
verification did not catch this because it ran the executable directly from a shell that
already had the right `PATH` inherited — the one launch method that does not reproduce a
real user's experience.

The CLI never hits this, since it is always run from an interactive shell where `PATH`
is already correct — so this fix belongs entirely to the GUI's entry point, not to
`tony.py`.

- [ ] **Step 1: Write the failing tests**

Create `tests/test_gui_main.py`:
...
Run: `pytest tests/test_gui_main.py -v`
Expected: FAIL — `ImportError: cannot import name 'augment_path' from 'gui.__main__'`

- [ ] **Step 3: Implement `augment_path` and wire it into `gui/__main__.py`**

Replace `gui/__main__.py` entirely:
...
The `# noqa: E402` marks the deliberately-late import for anyone running a linter over
this file — it must come after the `os.environ["PATH"]` line, not before, or the fix
would not be in place before `gui.app` (and anything it imports) has a chance to run.
...
Run: `QT_QPA_PLATFORM=offscreen pytest tests/ -v`
Expected: PASS — the full suite, unaffected, plus these 8 new tests. Nothing about this
change touches `gui/app.py` or any other module; it runs once, at process start, before
anything else imports.

- [ ] **Step 6: Rebuild the app and verify against the actual restricted-PATH scenario**

This is the step that must catch what Task 13's launch check missed - do not skip it or
substitute running the raw executable with an inherited shell PATH, which is exactly the
condition that let this bug through once already.

```bash
./build/build_app.sh
```

Then launch it with a deliberately minimal PATH, the way `launchd` actually would:

```bash
env -i PATH="/usr/bin:/bin:/usr/sbin:/sbin" HOME="$HOME" QT_QPA_PLATFORM=offscreen \
  "build/dist/Tonie Audio Updater.app/Contents/MacOS/Tonie Audio Updater" &
sleep 4
kill %1 2>/dev/null
```

Also verify directly, without the GUI, that the fix takes effect for the engine's own
checks under the same restricted PATH:

```bash
env -i PATH="/usr/bin:/bin:/usr/sbin:/sbin" HOME="$HOME" .venv-dev/bin/python - <<'PY'
import sys
sys.path.insert(0, ".")
import gui.__main__  # noqa: F401  (import alone runs the PATH fix)
import tony
tony.parse_args(["-i", "/tmp"])
print("check_ffmpeg():", tony.check_ffmpeg())
print("check_ytdlp():", tony.check_ytdlp())
PY
```

Expected: both print `True`, provided FFmpeg and yt-dlp are actually installed via
Homebrew or MacPorts on this machine (they are, per earlier verification in this plan).
Before this task's fix, this exact script prints `False` for both - that contrast is the
whole point of this step. Report the actual output of both commands in your report,
not just "it passed."

- [ ] **Step 7: Commit**

```bash
git add gui/__main__.py tests/test_gui_main.py
git commit -m "Find Homebrew-installed tools when launched from Finder, not just a shell"
```
